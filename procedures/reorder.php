		...
		if(!empty($folder_guid) && (!empty($parent_guid) || $parent_guid == 0)){
			...
			if(!empty($parent_guid)){
				if($parent = get_entity($parent_guid)){
					if($parent->getSubtype() != FILE_TREE_SUBTYPE){
						unset($parent_guid);
					}
				} else {
					unset($parent_guid);
			...
			if(($folder = get_entity($folder_guid)) && !is_null($parent_guid)){
				if(($folder->getSubtype() == FILE_TREE_SUBTYPE) && $folder->canEditMetadata("parent_guid")){
					// set new parent_guid
			...
			if(!empty($order) && !is_null($parent_guid)){
				foreach($order as $index => $order_guid){
					if($folder = get_entity($order_guid)){
						if(($folder->getSubtype() == FILE_TREE_SUBTYPE) && $folder->canEditMetadata("order")){
